...
class DataValidator:
    ...
    def validate_csv_data(self, csv_path: Path) -> Dict[str, Any]:
        ...
        """
        required_columns = ['id', 'symbol', 'strategy', 'entry_time', 
                          'exit_time', 'pnl_usd', 'status']
    ...
    def validate_polymarket_prices(
        ...
        if not (0.85 <= sum_price <= 1.05):
            issues.append(
                f"Price sum {sum_price:.4f} is suspicious "
                f"(expected 0.85-1.05)"
            )
