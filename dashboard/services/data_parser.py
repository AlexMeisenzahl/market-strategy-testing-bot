...
class DataParser:
            ...
            with open(self.trades_csv, "r", newline="", encoding="utf-8") as f:
                ...
                for row in reader:
                    try:
                        ...
                        # Extract pnl_usd once to avoid redundant lookups
                        pnl_usd_value = float(
                            row.get("pnl_usd") or row.get("profit_usd") or 0
                        )

                        trade = {
                            "id": int(row.get("id") or 0) if row.get("id") else 0,
                            "symbol": row.get("symbol")
                            or row.get("market", ""),  # Map market → symbol
                            ...
                            "pnl_usd": pnl_usd_value,  # Map profit_usd → pnl_usd
                            "pnl_pct": float(
                                row.get("pnl_pct") or row.get("profit_pct") or 0
                            ),  # Map profit_pct → pnl_pct
                            "status": row.get("status", "closed"),
                            "outcome": (
                                "win"
                                if pnl_usd_value > 0
                                else ("loss" if pnl_usd_value < 0 else "breakeven")
                            ),
                        }
                        trades.append(trade)
                ...
                for row in reader:
                    try:
                        ...
                        opportunity = {
                            ...
                            "action_taken": row.get("action_taken", "false").lower()
                            == "true",
            ...
            opportunities.append(
                {
                    ...
                    "outcome": (
                        "win"
                        if acted_on and (i % 3) != 0
                        else "loss" if acted_on else None
                    ),
                    "pnl_usd": (
                        round((10 + i % 20) * (1 if (i % 3) != 0 else -1), 2)
                        if acted_on
                        else None
                    ),
                }
    ...
    def calculate_arbitrage_profit_pct(
        ...
        assert (
            Decimal("0.85") <= sum_price <= Decimal("1.05")
    ...
    def prepare_daily_pnl_chart_data(
    ...
    def prepare_cumulative_pnl_chart_data(
        self, trades: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        return {"labels": [d.isoformat() for d in sorted_dates], "data": cumulative}
    ...
    def get_strategy_performance(self) -> Dict[str, Dict[str, Any]]:
        ...
        for strategy, trades in strategy_trades.items():
            ...
            win_rate = Decimal("0")
            if total_trades > 0:
                win_rate = (Decimal(str(wins)) / Decimal(str(total_trades))) * Decimal(
                    "100"
                )
            ...
            performance[strategy] = {
                ...
                "losses": losses,
                "win_rate": float(
                    win_rate.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
                ),
                "total_pnl": float(
