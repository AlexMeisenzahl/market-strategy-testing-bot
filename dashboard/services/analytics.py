...
class AnalyticsService:
    ...
    def calculate_sharpe_ratio(
        self, trades: List[Dict[str, Any]], risk_free_rate: float = 0.02
    ) -> float:
        """
    ...
    def calculate_max_drawdown(self, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            "max_drawdown": float(
                max_drawdown.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
            ),
            "max_drawdown_pct": float(
                max_drawdown_pct.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
            ),
            "peak_value": float(
                peak_value.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
            ),
            "trough_value": float(
                trough_value.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
            ),
        }
    ...
    def calculate_win_loss_ratio(self, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if losses:
            avg_loss = abs(
                sum(Decimal(str(t["pnl_usd"])) for t in losses) / len(losses)
            )
        else:
            avg_loss = Decimal("0")
        ...
        return {
            ...
            "avg_loss": float(
                avg_loss.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
            ),
            "win_loss_ratio": float(
                win_loss_ratio.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
            ),
            "total_wins": len(wins),
    ...
    def get_overview_stats(self) -> Dict[str, Any]:
        ...
        avg_duration = (
            sum(t["duration_minutes"] for t in trades) / len(trades) if trades else 0
        )
        ...
        best_strategy = (
            max(strategy_pnl.items(), key=lambda x: x[1])[0] if strategy_pnl else "N/A"
        )
        ...
        recent_trades = [
            t for t in trades if datetime.fromisoformat(t["entry_time"]) >= yesterday
        ]
        ...
        max_drawdown_info = self.calculate_max_drawdown(
            sorted(trades, key=lambda x: x["entry_time"])
        )
        ...
        return {
            ...
            "largest_loss": (
                round(min([t["pnl_usd"] for t in losses]), 2) if losses else 0
            ),
