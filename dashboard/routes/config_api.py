...
def update_config():
    ...
    try:
        ...
        if not updates:
            return (
                jsonify({"success": False, "error": "No configuration data provided"}),
                400,
            )
...
def get_config_section(section):
    ...
    try:
        ...
        if section not in config:
            return (
                jsonify({"success": False, "error": f"Section '{section}' not found"}),
                404,
            )

        return jsonify({"success": True, "section": section, "data": config[section]})
...
def reset_config():
    ...
    try:
        ...
        if not example_config_path.exists():
            return (
                jsonify(
                    {"success": False, "error": "Example configuration file not found"}
                ),
                404,
            )
        ...
        if not isinstance(strategies, dict):
            ...
            for strategy_name, strategy_config in strategies.items():
                if not isinstance(strategy_config, dict):
                    errors.append(
                        f"Strategy '{strategy_name}' configuration must be a dictionary"
                    )
                    continue

                # Validate enabled flag
                if "enabled" in strategy_config and not isinstance(
                    strategy_config["enabled"], bool
                ):
                    errors.append(
                        f"Strategy '{strategy_name}' enabled flag must be boolean"
                    )
