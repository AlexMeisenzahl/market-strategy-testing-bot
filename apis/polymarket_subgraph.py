...
class PolymarketSubgraph:
            ...
            if response.status_code == 200:
                ...
                if self.logger:
                    self.logger.log_error(
                        f"Subgraph API error: HTTP {response.status_code}"
                    )
                return None
    ...
    def query_trades(
        self, market_id: Optional[str] = None, first: int = 10
    ) -> List[Dict]:
        """
        Query recent trades
    ...
    def search_markets_by_topic(self, topic: str, limit: int = 20) -> List[Dict]:
        ...
        """
        return self.query_markets(active=True, first=limit, question_contains=topic)

    def get_high_volume_markets(
        self, min_volume_usd: float = 10000, limit: int = 20
    ) -> List[Dict]:
        """
