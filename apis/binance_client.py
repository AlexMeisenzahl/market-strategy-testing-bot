...
class BinanceClient:
    ...
    def get_price(self, symbol: str) -> Optional[float]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if self.logger:
                    self.logger.log_error(f"Binance API error: HTTP {response.status_code}")
                return None
    ...
    def stream_prices(self, symbols: List[str], callback: Callable[[Dict], None], 
        ...
        try:
            ...
            if self.logger:
                self.logger.log_error("websocket-client not installed. Run: pip install websocket-client")
            return False
        ...
        def on_open(ws):
            if self.logger:
                self.logger.log_info(f"WebSocket connected - streaming {len(symbols)} symbols")
