...
class ArbitrageBot:
    ...
    def update_dashboard(self, layout: Layout) -> None:
        ...
        header_text.append(
            ...
            style="bold green" if status_text.startswith("✓") else "bold yellow",
        )
        ...
        status_text = (
            f"Runtime: {runtime_str}                    Last Update: {last_update_str}"
        )
        ...
        seconds_since_check = int(
            (datetime.now() - self.last_connection_check).total_seconds()
        )
        ...
        layout["connection"].update(
            Panel(conn_table, title="CONNECTION", box=box.ROUNDED)
        )
        ...
        usage_text = Text(
            f"{bar} {rate_status['current']}/{rate_status['max']} ({rate_pct:.0f}%)"
        )
        ...
        layout["rate_limit"].update(
            Panel(rate_table, title="API RATE LIMIT", box=box.ROUNDED)
        )
        ...
        try:
            ...
            strategy_stats["opportunities_found"] = sum(
                s.get("opportunities_found", 0) for s in all_strategy_stats.values()
            )
        except Exception:
            pass
        ...
        trade_table.add_row(
            "Opportunities Found:", str(strategy_stats["opportunities_found"])
        )
        trade_table.add_row(
            "Paper Trades Executed:", str(trade_stats["trades_executed"])
        )

        profit_text = (
            f"+${trade_stats['total_profit']:.2f} ({trade_stats['return_percentage']:.1f}% return)"
            if trade_stats["total_profit"] > 0
            else "$0.00 (0% return)"
        )
        ...
        layout["activity"].update(
            Panel(activity_text.rstrip(), title="LATEST ACTIVITY", box=box.ROUNDED)
        )
        ...
        layout["alerts"].update(
            Panel(
                alert_text.rstrip(), title="ALERTS", box=box.ROUNDED, style=alert_style
            )
        )
    ...
    def scan_markets(self) -> None:
        ...
        try:
            ...
            if (
            ...
            ).total_seconds() > self.config.get("connection_check_interval", 10):
                ...
                if not is_healthy and self.config.get(
            ...
            opportunities = self.strategy_manager.find_opportunities(
                markets_to_scan, prices_dict
            )
            ...
            for market in markets_to_scan:
                ...
                if market_id in prices_dict:
                    ...
                    opp = next(
                        (o for o in opportunities if o.get("market_id") == market_id),
                        None,
                    )
                    ...
                    if opp:
                        ...
                        if self.config.get("alert_on_opportunities", True):
                            self.add_alert(
                                f"💰 {market['question'][:40]}... ({profit_margin:.1f}% profit)"
                            )
                            self.notifier.alert_opportunity_found(
                                market["question"], profit_margin
                            )
                                ...
                                if trade:
                                    ...
                                    if feature_flags.is_enabled("prometheus_metrics"):
                                        ...
                                        metrics.update_profit(
                                            trader_stats["total_profit"]
                                        )
                                        metrics.update_trades_count(
                        ...
                        # No opportunity
                        self.add_activity(
                            f"✓ Scanned: {market['question']} (no opportunity)"
                        )
    ...
    def _get_demo_markets(self) -> list:
        ...
        for market_name in configured_markets:
            ...
            if not any(m["id"] == market_id for m in demo_markets):
                demo_markets.append({"id": market_id, "question": market_name})
        ...
        try:
            ...
            if not markets:
                self.logger.log_warning(
                    "No markets returned from API, using demo markets"
                )
                return self._get_demo_markets()
            ...
            for market in markets:
                ...
                if configured_keywords:
                    ...
                    if not any(
                        keyword.lower() in question for keyword in configured_keywords
                    ):
                        continue
            ...
            if not filtered:
                self.logger.log_warning("No markets passed filters, using demo markets")
                return self._get_demo_markets()
    ...
    def run(self) -> None:
        ...
        self.console.clear()
        self.console.print(
            "[bold cyan]Starting Polymarket Arbitrage Bot...[/bold cyan]"
        )
        ...
        if enabled_strategies:
            self.console.print(
                f"[green]Enabled strategies: {', '.join(enabled_strategies)}[/green]"
            )
        ...
        try:
            with Live(
                layout, console=self.console, refresh_per_second=1, screen=True
            ) as live:
                while self.running:
                    # Check kill switch
                    if self.check_kill_switch():
            ...
            self.console.print(
                f"[green]Total paper profit: ${self.trader.get_statistics()['total_profit']:.2f}[/green]"
            )
