...
class CompetitionMonitor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.high_competition_threshold = comp_config.get(
            "high_competition_lifespan", 1.0
        )  # < 1s
        self.low_competition_threshold = comp_config.get(
            "low_competition_lifespan", 5.0
        )  # > 5s
        self.snipe_threshold = comp_config.get("snipe_threshold", 0.5)  # < 0.5s = snipe
        self.history_window = comp_config.get(
    ...
    def track_opportunity(self, opportunity_id: str, market: str) -> OpportunityTracker:
        ...
        tracker = OpportunityTracker(
            opportunity_id=opportunity_id, market=market, detected_at=datetime.now()
        )
    ...
    def detect_snipe_patterns(self) -> Dict[str, Any]:
        ...
        snipe_times = [
            lifespan
            for lifespan in self.lifespan_history
            if lifespan < self.snipe_threshold
        ]

        snipe_rate = (
            len(snipe_times) / len(self.lifespan_history)
            if self.lifespan_history
            else 0
        )
    ...
    def get_competition_report(self) -> str:
        ...
        report += (
            f"  Competition Indicator: {lifespan_analysis['competition_indicator']}\n"
        )
        ...
        report += (
            f"  Competition Indicator: {snipe_analysis['competition_indicator']}\n"
        )
