...
class BotRunner:
    ...
    def _fetch_markets(self) -> List[Dict[str, Any]]:
        """
        Fetch current markets from Polymarket API or use mock data
    ...
    def _process_opportunities(
        self, 
        all_opportunities: Dict[str, List[Any]]
    ) -> None:
        """
    ...
    def _execute_trades(self, all_opportunities: Dict[str, List[Any]]) -> None:
        ...
        Args:
            all_opportunities: Dictionary mapping strategy name to list of opportunities
        """
        trades_executed = self.strategy_manager.execute_best_opportunities(all_opportunities)
    ...
    def run_cycle(self) -> None:
        ...
        try:
            ...
            self.logger.log_warning(f"🔄 Cycle #{self.cycles_completed} - {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}")
            ...
            all_opportunities = self.strategy_manager.run_all_strategies(markets, prices_dict)
            ...
            # 6. Log cycle summary
            total_opps_this_cycle = sum(len(opps) for opps in all_opportunities.values())
            self.logger.log_warning(
    ...
    def run(self) -> None:
        ...
        self.logger.log_warning(f"💰 Total Capital: ${self.strategy_manager.total_capital}")
        self.logger.log_warning(f"📝 Paper Trading Mode: {'ENABLED' if self.config.get('paper_trading', True) else 'DISABLED'}")
    ...
    def shutdown(self) -> None:
        ...
        self.logger.log_warning(f"🔍 Opportunities found: {self.total_opportunities_found}")
        self.logger.log_warning(f"⚡ Trades executed: {self.total_trades_executed}")
        self.logger.log_warning(f"⏭️  Opportunities skipped: {self.total_opportunities_skipped}")
        self.logger.log_warning("=" * 60)
