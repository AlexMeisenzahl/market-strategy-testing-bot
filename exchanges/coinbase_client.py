...
class CoinbaseClient:
    ...
    def get_price(self, symbol: str) -> Optional[Decimal]:
        ...
        if cached:
            if self.logger:
                self.logger.log_info(f"Coinbase: {currency_pair} = ${cached['price']} (cached)")
            return cached['price']
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'data' in data and 'amount' in data['data']:
                    ...
                    if self.logger:
                        self.logger.log_warning(f"Coinbase: No price data for {currency_pair}")
                    return None
            else:
                if self.logger:
                    self.logger.log_error(f"Coinbase API error: HTTP {response.status_code}")
                return None
