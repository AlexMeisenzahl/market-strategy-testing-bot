...
class PriceAggregator:
    ...
    def _remove_outliers(
        self, data: List[Tuple[float, float, str]]
        ...
        # Filter out outliers
        filtered = [
            data[i] for i in range(len(data)) if z_scores[i] < self.outlier_threshold
        ]

        return filtered if filtered else data  # Return original if all filtered

    def _weighted_consensus(self, data: List[Tuple[float, float, str]]) -> float:
        """
        ...
        if total_volume > 0:
            volume_weight_price = sum(p * v / total_volume for p, v, _ in data)
        else:
    ...
    def _calculate_confidence(
        self, cleaned_data: List[Tuple[float, float, str]], all_prices: List[float]
    ) -> int:
        """
    ...
    def detect_anomaly(
        ...
        if not historical_prices or len(historical_prices) < 3:
            return {"is_anomaly": False, "reason": "Insufficient history", "z_score": 0}
    ...
    def calculate_quality_score(
        ...
        if aggregation_result["price_spread_pct"] > 5:
            issues.append(
                f"High price spread: {aggregation_result['price_spread_pct']:.1f}%"
            )
            score -= 15
