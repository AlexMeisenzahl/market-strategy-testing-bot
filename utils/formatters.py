...
def format_duration(
    ...
    if hours > 0:
        parts.append(f"{hours}h" if compact else f"{hours} hour{'s' if hours != 1 else ''}")
    if minutes > 0:
        parts.append(f"{minutes}m" if compact else f"{minutes} minute{'s' if minutes != 1 else ''}")
    if show_seconds and (secs > 0 or not parts):
        parts.append(f"{secs}s" if compact else f"{secs} second{'s' if secs != 1 else ''}")
