...
class WeatherTradingStrategy:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not api_key or api_key == "${NOAA_API_KEY}":
            self.logger.log_warning("NOAA API key not configured, weather trading disabled")
    ...
    def find_opportunities(self, markets: List[Dict]) -> List[WeatherOpportunity]:
        ...
        for market in markets:
            try:
                ...
                self.logger.log_error(f"Error analyzing market {market.get('question', 'Unknown')}: {e}")
                continue
        
        return opportunities
    
    def _parse_weather_market(self, market: Dict) -> Tuple[Optional[str], Optional[Dict]]:
        """
    ...
    def analyze(self, market_data: Dict, weather_params: Dict) -> Optional[WeatherOpportunity]:
        """
        ...
        market_type = 'temperature' if 'threshold' in weather_params else 'precipitation'
        location = weather_params['location']
        target_date = weather_params['target_date']
        
        # Get weather forecast
        if market_type == 'temperature':
            weather_data = self.weather_api.get_temperature_forecast(location, target_date)
            ...
            precip_prob = self.weather_api.get_precipitation_probability(location, target_date)
    ...
    def _calculate_bet_size(self, confidence: float, price: float) -> float:
        ...
        bet_size = kelly_fraction * self.max_bet_size * 4  # Assuming max_bet_size is 25% of bankroll
    ...
    def should_exit(self, position: Dict, current_data: Dict) -> Tuple[bool, str]:
        ...
        if 'precipitation_probability' in current_weather and 'precipitation_probability' in original_weather:
            prob_change = abs(
                current_weather['precipitation_probability'] - 
                original_weather['precipitation_probability']
            )
            if prob_change > 30:  # More than 30% change
                return True, f"Precipitation probability changed significantly ({prob_change}%)"
