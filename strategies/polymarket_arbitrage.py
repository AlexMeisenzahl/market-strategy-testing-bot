...
class PolymarketArbitrageStrategy:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.cross_exchange_enabled = arb_types.get("cross_exchange", {}).get("enabled", True)
        self.reality_based_enabled = arb_types.get("reality_based", {}).get("enabled", True)
        
        # Risk management
        self.max_position_size = config.get("max_daily_loss", 100) * 0.1  # 10% of max loss
    ...
    def _detect_simple_arbitrage(
        ...
        position_size = self._calculate_position_size(
            profit_margin_pct, liquidity_data
        )
    ...
    def close_position(
        ...
        if self.opportunities_executed > 0:
            wins = sum(
                1 for p in self.historical_opportunities
                if p.get("profit", 0) > 0
            )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        """
        total_positions = len(self.historical_opportunities) + len(self.active_positions)
