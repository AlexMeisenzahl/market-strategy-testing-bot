...
from strategies.statistical_arb_strategy import (
    ...
    def _load_strategies(self) -> None:
        ...
        if arb_config.get("enabled", True):
            try:
                ...
                strategy_config = {
                    "min_profit_margin": arb_config.get(
                ...
                self.strategies["polymarket_arbitrage"] = ArbitrageStrategy(
                    strategy_config
                )
        ...
        if momentum_config.get("enabled", False):
            try:
                strategy_config = {
                    "symbols": momentum_config.get(
                        "symbols", ["BTC", "ETH", "SOL", "XRP"]
                    ),
                    "momentum_min_price_change_5m": momentum_config.get(
                        "min_price_change_5m", 2.0
                    ),
                    "momentum_min_volume_change": momentum_config.get(
        ...
        if news_config.get("enabled", False):
            try:
                strategy_config = {
                    "news_volume_spike_threshold": news_config.get(
                        "volume_spike_threshold", 300.0
                    ),
                    "news_min_price_movement": news_config.get(
        ...
        if stat_arb_config.get("enabled", False):
            try:
                strategy_config = {
                    "stat_arb_min_correlation": stat_arb_config.get(
                        "min_correlation", 0.7
                    ),
                    "stat_arb_z_score_threshold": stat_arb_config.get(
                        "z_score_threshold", 2.0
                    ),
                    "stat_arb_max_holding_time": stat_arb_config.get(
                        "max_holding_time", 3600
                    ),
                }

                self.strategies["crypto_statistical_arb"] = StatisticalArbStrategy(
                    strategy_config
                )
        ...
        # Load Weather Trading Strategy
        weather_config = strategies_config.get("weather_trading", {})
        if weather_config.get("enabled", False):
            try:
                ...
                self.logger.log_error(
                    f"Failed to load Weather Trading strategy: {str(e)}"
                )

        # Load BTC Arbitrage Strategy
        btc_arb_config = strategies_config.get("btc_arbitrage", {})
        if btc_arb_config.get("enabled", False):
            try:
                strategy_config = {
                ...
                self.strategies["btc_arbitrage"] = BTCArbitrageStrategy(strategy_config)
                self.logger.log_info("Loaded BTC Arbitrage strategy")
            except Exception as e:
                self.logger.log_error(
                    f"Failed to load BTC Arbitrage strategy: {str(e)}"
                )
    ...
    def find_opportunities(
        ...
        for strategy_name, strategy in self.strategies.items():
            try:
                ...
                self.logger.log_error(f"Error running {strategy_name}: {str(e)}")
                continue
