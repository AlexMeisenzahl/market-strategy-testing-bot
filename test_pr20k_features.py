...
def test_alert_manager():
    ...
    try:
        ...
        alert_id = alert_manager.create_alert(
            "price", {"symbol": "BTC", "threshold": 100000, "direction": "above"}
        )
...
def test_database_models():
    ...
    try:
        ...
        price_id = CryptoPriceHistory.insert(
            symbol="bitcoin", price_usd=50000.0, volume=1000000.0
        )
        print(f"✓ Price history inserted: #{price_id}")

        # Test TradeJournal
        journal_id = TradeJournal.create(entry_reason="Test trade", confidence_level=8)
        print(f"✓ Journal entry created: #{journal_id}")

        # Test Alert
        alert_id = Alert.create("price", '{"symbol": "BTC", "threshold": 100000}')
...
def test_portfolio_manager_enhancements():
    ...
    try:
        ...
        high_vol_size = portfolio_manager.calculate_position_size_volatility_adjusted(
            strategy="test", entry_price=100.0, volatility=10.0  # High volatility
        )

        low_vol_size = portfolio_manager.calculate_position_size_volatility_adjusted(
            strategy="test", entry_price=100.0, volatility=2.0  # Low volatility
        )
