...
class ConfigLoader:
    ...
    def _load_config(self):
        ...
        logger.info(f"Configuration loaded from: defaults, YAML: {bool(self.config_path)}, ENV: True")
    ...
    def _convert_value(self, value: str, key: str) -> Any:
        ...
        # Boolean conversion
        if key in ["debug", "paper_trading", "rate_limit_enabled"] or key.startswith("feature_"):
            return value.lower() in ["true", "1", "yes", "on"]
        
        # Integer conversion
        if key in ["dashboard_port", "prometheus_port", "grafana_port", "redis_port", 
                   "workers", "worker_connections", "request_timeout", "jwt_expiration_hours"]:
            try:
                return int(value)
            except ValueError:
                logger.warning(f"Could not convert {key}={value} to int, using as string")
                return value
        ...
        if key in ["min_profit_margin", "sentry_traces_sample_rate"]:
            try:
                ...
                logger.warning(f"Could not convert {key}={value} to float, using as string")
                return value
    ...
    def get_secret_key(self) -> str:
        ...
        if secret == self.DEFAULTS["secret_key"] and self.is_production():
            logger.warning("Using default secret key in production! Please set SECRET_KEY environment variable.")
        return secret
