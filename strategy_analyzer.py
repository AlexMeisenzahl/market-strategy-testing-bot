...
class StrategyAnalyzer:
    ...
    def compare_strategies(
        ...
        if not performance_data:
            return {"error": "No performance data provided", "strategies": {}}
    ...
    def _analyze_single_strategy(
        ...
        avg_win = (
            sum(t.get("profit", 0) for t in winning_trades) / len(winning_trades)
            if winning_trades
            else 0
        )
        avg_loss = (
            sum(t.get("profit", 0) for t in losing_trades) / len(losing_trades)
            if losing_trades
            else 0
        )
        ...
        return {
            ...
            "profit_factor": (
                abs(
                    sum(t.get("profit", 0) for t in winning_trades)
                    / sum(t.get("profit", 0) for t in losing_trades)
                )
                if losing_trades and sum(t.get("profit", 0) for t in losing_trades) != 0
                else float("inf")
            ),
            "sharpe_ratio": sharpe_ratio,
            "consistency_score": consistency_score,
    ...
    def _calculate_overall_score(self, metrics: Dict[str, float]) -> float:
        ...
        normalized = {
            ...
            "total_profit": min(
                100, max(0, metrics["total_profit"] * 10)
            ),  # Assuming $10 profit = 100 score
            "sharpe_ratio": min(
                100, max(0, (metrics["sharpe_ratio"] + 2) * 25)
            ),  # Sharpe of 2 = 100 score
            "consistency_score": metrics["consistency_score"],
            "frequency": min(
                100, metrics["frequency"] * 20
            ),  # 5 trades/day = 100 score
        }
    ...
    def generate_allocation_recommendation(
        self, analysis: Dict[str, Any], total_capital: float
    ) -> Dict[str, Any]:
        """
        ...
        if not valid_strategies:
            return {"error": "No valid strategies for allocation", "allocations": {}}
        ...
        if total_score == 0:
            ...
            # Normalize to 100%
            total_allocated_pct = (
                sum(a["percentage"] for a in allocations.values()) / 100
            )
    ...
    def _generate_allocation_reasoning(
        self, metrics: Dict[str, Any], allocation_pct: float
    ) -> str:
        """
    ...
    def _generate_summary(
        ...
        """
        valid_strategies = [
            s for s in strategies.values() if not s.get("insufficient_data", False)
        ]
        ...
        return {
            ...
            "avg_win_rate": sum(s.get("win_rate", 0) for s in valid_strategies)
            / len(valid_strategies),
            "avg_sharpe_ratio": sum(s.get("sharpe_ratio", 0) for s in valid_strategies)
            / len(valid_strategies),
            "best_strategy": max(
                valid_strategies, key=lambda s: s.get("overall_score", 0)
            )["strategy_name"],
            "worst_strategy": min(
                valid_strategies, key=lambda s: s.get("overall_score", 0)
            )["strategy_name"],
    ...
    def _log_comparison(self, analysis: Dict[str, Any]) -> None:
        ...
        self.logger.log_warning(f"Combined profit: ${summary['total_profit']:.2f}")
        self.logger.log_warning(
