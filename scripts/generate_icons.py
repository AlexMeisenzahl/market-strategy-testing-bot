...
def create_icon(size, output_path):
    ...
    for x, y in bars:
        ...
        draw.rectangle([x, y, x + bar_width, base_y], fill=ACCENT_COLOR)
        # Highlight on top
        highlight_height = bar_width // 3
        draw.rectangle([x, y, x + bar_width, y + highlight_height], fill="#22c55e")
    ...
    for i in range(len(trend_points) - 1):
        draw.line(
            [trend_points[i], trend_points[i + 1]], fill=line_color, width=line_width
        )
...
def create_splash_screen(width, height, name, output_path):
    ...
    for i in range(4):
        ...
        draw.rectangle([bar_x, bar_y, bar_x + bar_width, base_y], fill=ACCENT_COLOR)
    ...
    draw.text((text_x, text_y), app_name, fill=TEXT_COLOR, font=font)
