...
class RiskAdjustedPositionSizer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.base_allocation_pct = config.get('position_sizing', {}).get('base_allocation_pct', 0.05)
        
        # Hard limits
        self.max_allocation_pct = config.get('position_sizing', {}).get('max_allocation_pct', 0.10)
        self.min_position_size = config.get('position_sizing', {}).get('min_position_size', 1.0)
        
        # Multipliers for different factors
        multipliers = config.get('position_sizing', {}).get('multipliers', {})
        self.profit_margin_multipliers = multipliers.get('profit_margin', {
    ...
    def calculate_position_size(
        ...
        if stats:
            ...
            liquidity_mult, liquidity_reasoning = self._assess_liquidity(opportunity, stats)
            volatility_mult, volatility_reasoning = self._assess_volatility(opportunity, stats)
        else:
    ...
    def _assess_liquidity(self, opportunity: ArbitrageOpportunity, stats: Dict[str, Any]) -> tuple:
        """
        ...
        market_liquidity = stats.get('market_liquidity', {}).get(opportunity.market_id, 'medium')
    ...
    def _assess_volatility(self, opportunity: ArbitrageOpportunity, stats: Dict[str, Any]) -> tuple:
        """
        ...
        market_volatility = stats.get('market_volatility', {}).get(opportunity.market_id, 'normal')
    ...
    def _log_sizing_decision(self, opportunity: ArbitrageOpportunity, details: Dict[str, Any]) -> None:
        """
