...
class ExitManager:
    ...
    def check_exit_conditions(self, position: Position,
                             current_price: float = None) -> Optional[str]:
        """
    ...
    def execute_exit(self, position: Position, reason: str,
    ...
    def monitor_all_positions(self, current_prices: Dict[str, float] = None) -> List[Dict]:
        """
    ...
    def set_position_config(self, position_id: str, stop_loss_pct: float = None,
                          take_profit_pct: float = None, max_hold_hours: int = None,
    ...
    def get_exit_statistics(self) -> Dict:
        ...
        stop_loss_exits = sum(1 for e in self.exit_history if 'stop_loss' in e['reason'])
        take_profit_exits = sum(1 for e in self.exit_history if 'take_profit' in e['reason'])
        time_based_exits = sum(1 for e in self.exit_history if 'max_hold' in e['reason'])
        trailing_stop_exits = sum(1 for e in self.exit_history if 'trailing_stop' in e['reason'])
        
        avg_profit_pct = sum(e['profit_pct'] for e in self.exit_history) / len(self.exit_history)
    ...
    def update_defaults(self, stop_loss_pct: float = None,
