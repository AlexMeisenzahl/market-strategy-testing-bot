...
class NotificationService:
    ...
    def send_notification(
        self, notification_type: str, data: Dict[str, Any], user_id: int = 1
    ) -> bool:
        """
        ...
        try:
            ...
            preferences = NotificationPreference.get_for_type(
            ...
            if not preferences:
                ...
                channel_ids = [
                    ...
                    if p.get("enabled") and p.get("channel_id")
                ]
                ...
                # Apply filters
                channels = [
                    c for c in channels if self._passes_filters(data, preferences, c)
                ]
    ...
    def _passes_filters(
        ...
        pref = next(
            (p for p in preferences if p.get("channel_id") == channel["id"]), None
        )
        if not pref:
        ...
        if min_confidence:
            ...
            if (
                ...
                or min_confidence not in confidence_order
            ):
                return False
    ...
    def _send_to_channel(
        self, channel: Dict[str, Any], notification_type: str, data: Dict[str, Any]
    ) -> bool:
        """
    ...
    def _send_discord(
        ...
        try:
            ...
            if "market" in data:
                embed["fields"].append(
                    {"name": "Market", "value": data["market"], "inline": False}
                )
            if "profit" in data:
                embed["fields"].append(
                    {
            ...
            if "confidence" in data:
                embed["fields"].append(
                    {
                        "name": "Confidence",
                        "value": data["confidence"].replace("_", " ").title(),
                        "inline": True,
                    }
                )
            if "strategy" in data:
                embed["fields"].append(
                    {"name": "Strategy", "value": data["strategy"], "inline": True}
                )
            if "arbitrage_type" in data:
                embed["fields"].append(
                    {"name": "Type", "value": data["arbitrage_type"], "inline": True}
                )
            if "reason" in data:
                embed["fields"].append(
    ...
    def _format_email_html(self, notification_type: str, data: Dict[str, Any]) -> str:
        ...
        if "market" in data:
            rows.append(
                f"<tr><td><strong>Market:</strong></td><td>{data['market']}</td></tr>"
            )
        if "profit" in data:
            rows.append(
                f"<tr><td><strong>Profit:</strong></td><td>${data['profit']:.2f}</td></tr>"
            )
        if "confidence" in data:
            conf = data["confidence"].replace("_", " ").title()
            rows.append(
                f"<tr><td><strong>Confidence:</strong></td><td>{conf}</td></tr>"
            )
        if "strategy" in data:
            rows.append(
                f"<tr><td><strong>Strategy:</strong></td><td>{data['strategy']}</td></tr>"
            )
        if "arbitrage_type" in data:
            rows.append(
                f"<tr><td><strong>Type:</strong></td><td>{data['arbitrage_type']}</td></tr>"
            )
    ...
    def test_channel(self, channel_type: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        channel = {"channel_type": channel_type, **config}

        try:
            success = self._send_to_channel(
                channel, NotificationType.SYSTEM_HEALTH_CHECK, test_data
            )
