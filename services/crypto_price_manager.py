...
class CryptoPriceManager:
    ...
    def __init__(self, logger=None, log_dir: str = "logs", config: Dict = None):
        ...
        self.coingecko = (
            CoinGeckoClient(logger=logger)
            if crypto_config.get("coingecko", {}).get("enabled", True)
            else None
        )
        self.binance = (
            BinanceClient(logger=logger)
            if crypto_config.get("binance", {}).get("enabled", True)
            else None
        )
        self.coinbase = (
            CoinbaseClient(logger=logger)
            if crypto_config.get("coinbase", {}).get("enabled", True)
            else None
        )
                ...
                try:
                    ...
                    if self.logger:
                        self.logger.log_warning(
                            f"Failed to fetch from {source}: {str(e)}"
                        )
        ...
        if len(prices) < self.min_sources:
            if self.logger:
                self.logger.log_error(
                    f"Insufficient price sources for {symbol}: only {len(prices)} available"
                )
            return None
    ...
    def check_price_alert(
        self, symbol: str, threshold: Decimal, direction: str
    ) -> bool:
        """
    ...
    def health_check(self) -> Dict[str, bool]:
        ...
        if self.logger:
            self.logger.log_info(
                f"Price sources health: {healthy_count}/{total_count} healthy"
            )
