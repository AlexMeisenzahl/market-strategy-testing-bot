...
class PriceAlertManager:
    ...
    def check_alerts(self) -> List[Dict]:
        ...
        for alert in self.alerts:
            ...
            if is_triggered:
                ...
                if self.logger:
                    if alert_type == 'above':
                        self.logger.log_info(f"🚨 PRICE ALERT: {symbol} crossed above ${threshold:,.0f} (now ${current_price:,.0f})")
                    elif alert_type == 'below':
                        self.logger.log_info(f"🚨 PRICE ALERT: {symbol} dropped below ${threshold:,.0f} (now ${current_price:,.0f})")
                    elif alert_type == 'change_pct':
                        self.logger.log_info(f"🚨 PRICE ALERT: {symbol} changed {change_pct:+.1f}% (now ${current_price:,.0f})")
        
        return triggered
    
    def _should_reset_alert(self, alert_type: str, current_price: Decimal, threshold: Decimal) -> bool:
        """
    ...
    def add_alert(self, symbol: str, alert_type: str, threshold: float, notification: bool = True) -> None:
        """
        Add a new alert
        ...
        for existing in self.alerts:
            if (existing['symbol'] == symbol and 
                ...
                abs(existing['threshold'] - threshold) < 0.01):
        ...
        if self.logger:
            self.logger.log_info(f"Added price alert: {symbol} {alert_type} {threshold}")
    ...
    def remove_alert(self, symbol: str, alert_type: str, threshold: float) -> bool:
        ...
        for i, alert in enumerate(self.alerts):
            if (alert['symbol'] == symbol and 
                ...
                abs(alert['threshold'] - threshold) < 0.01):
                self.alerts.pop(i)
                
                if self.logger:
                    self.logger.log_info(f"Removed price alert: {symbol} {alert_type} {threshold}")
