...
class MarketAnalytics:
    ...
    def _calculate_market_metrics(
        ...
        best_trade = max(Decimal(str(t['pnl_usd'])) for t in trades) if trades else Decimal('0')
        worst_trade = min(Decimal(str(t['pnl_usd'])) for t in trades) if trades else Decimal('0')
        
        # Frequency: (market_trades / total_trades) × 100
        frequency = (Decimal(total_trades) / Decimal(total_trade_count) * Decimal('100')).quantize(
            Decimal('0.01'), rounding=ROUND_HALF_UP
        ) if total_trade_count > 0 else Decimal('0')
