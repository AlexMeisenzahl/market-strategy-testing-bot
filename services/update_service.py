...
class UpdateService:
    ...
    def _is_locked(self) -> Tuple[bool, Optional[Dict]]:
        ...
        try:
            ...
            if age > 1800:  # 30 minutes
                logger.log_warning(
                    f"Found stale lock (age: {age}s), considering it invalid"
                )
    ...
    def _update_progress(
        self, status: UpdateStatus, progress: int, message: str, logs: List[str] = None
    ):
    ...
    def pre_flight_checks(self) -> Tuple[bool, List[Dict]]:
        ...
        try:
            ...
            checks.append(
                {"name": "Git installed", "passed": True, "message": "Git is available"}
            )
        except Exception as e:
            checks.append(
                {
            ...
            checks.append(
                {
                    ...
                    "passed": True,
                    "message": "Internet is accessible",
                }
            )
        except Exception:
            checks.append(
            ...
            checks.append(
                {
                    "name": "GitHub accessible",
                    "passed": False,
                    "message": f"Cannot reach GitHub: {e}",
                }
            )
            all_passed = False
            ...
            checks.append(
                {
                    "name": "No concurrent updates",
                    "passed": True,
                    "message": "No other updates running",
                }
            )
        ...
        try:
                    ...
                    checks.append(
                        {
                            "name": "Git status",
                            "passed": True,
                            "message": "Local changes will be stashed",
                        }
                    )
                else:
                    checks.append(
            ...
            checks.append(
                {"name": "Git status", "passed": False, "message": f"Git error: {e}"}
            )
            all_passed = False
    ...
    def health_check(self) -> Tuple[bool, List[Dict]]:
        ...
        if self.process_manager.is_bot_running():
            checks.append(
                {"name": "Bot process", "passed": True, "message": "Bot is running"}
            )
        else:
            checks.append(
                {"name": "Bot process", "passed": False, "message": "Bot not running"}
            )
            all_passed = False
                ...
                checks.append(
                    {
                        "name": "Config file",
                        "passed": True,
                        "message": "Config loads correctly",
                    }
                )
            else:
                checks.append(
    ...
    def rollback(self, backup_name: str = None) -> bool:
        ...
        try:
            ...
            time.sleep(5)
            if (
                self.process_manager.is_bot_running()
                and self.process_manager.is_dashboard_running()
            ):
    ...
    def perform_update(self) -> Dict:
        ...
        try:
            ...
            self._update_progress(
                UpdateStatus.CHECKING, 5, "Running pre-flight checks...", logs
            )
            ...
            self._update_progress(
                UpdateStatus.BACKING_UP, 15, "Creating backup...", logs
            )
            ...
            self._update_progress(
                UpdateStatus.DOWNLOADING, 30, "Downloading from GitHub...", logs
            )
            ...
            if not download_success:
                ...
                self._update_progress(
                    UpdateStatus.ROLLING_BACK, 0, "Rolling back...", logs
                )
            ...
            self._update_progress(
                UpdateStatus.INSTALLING, 50, "Installing dependencies...", logs
            )
            ...
            self._update_progress(
                UpdateStatus.RESTARTING, 70, "Restarting processes...", logs
            )
            ...
            self._update_progress(
                UpdateStatus.VERIFYING, 85, "Running health checks...", logs
            )
