...
# Trading metrics
opportunities_found = Counter('opportunities_found_total', 'Total opportunities found', ['strategy'])
opportunities_missed = Counter('opportunities_missed_total', 'Total opportunities missed', ['reason'])
trades_executed = Counter('trades_executed_total', 'Total trades executed', ['strategy'])
...
api_failures = Counter('api_failures_total', 'Total API failures', ['service', 'error_type'])
api_latency = Histogram('api_latency_seconds', 'API call latency', ['service', 'endpoint'])

# Rate limiting metrics
rate_limit_hits = Counter('rate_limit_hits_total', 'Rate limit hits', ['service'])
rate_limit_usage = Gauge('rate_limit_usage_percent', 'Rate limit usage percentage', ['service'])

# Health metrics
connection_status = Gauge('connection_status', 'Connection health status (1=healthy, 0=unhealthy)', ['service'])
last_successful_scan = Gauge('last_successful_scan_timestamp', 'Timestamp of last successful market scan')

# Error metrics
bot_errors = Counter('bot_errors_total', 'Total bot errors', ['error_type'])
notification_failures = Counter('notification_failures_total', 'Notification failures', ['channel'])
