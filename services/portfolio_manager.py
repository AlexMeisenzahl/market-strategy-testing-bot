...
class PortfolioManager:
    ...
    def allocate_capital(self, size: float) -> bool:
        ...
        if size > self.cash_balance:
            self.logger.warning(
                f"Insufficient funds: ${size:.2f} requested, ${self.cash_balance:.2f} available"
            )
            return False
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        closed_positions = position_tracker.get_closed_positions()
        winning_trades = sum(
            1 for p in closed_positions if p.actual_profit and p.actual_profit > 0
        )
        win_rate = (
            (winning_trades / len(closed_positions) * 100) if closed_positions else 0.0
        )
        ...
        sharpe_ratio = (
            returns / 10.0 if returns > 0 else 0.0
        )  # Normalized by typical std dev estimate
    ...
    def get_strategy_performance(self) -> Dict[str, Dict[str, Any]]:
        ...
        for strategy, data in strategies.items():
            ...
            if closed > 0:
                winning = sum(
                    1
                    for p in position_tracker.get_positions_by_strategy(strategy)
    ...
    def get_daily_pnl(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        for i in range(days):
            ...
            day_positions = [
                p
                for p in position_tracker.get_closed_positions()
                if p.exit_time and p.exit_time.date() == date.date()
            ]

            day_pnl = sum(
                p.actual_profit for p in day_positions if p.actual_profit is not None
            )
