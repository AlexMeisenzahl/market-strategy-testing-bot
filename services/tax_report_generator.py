...
class TaxReportGenerator:
    ...
    def generate_irs_8949_csv(self, trades: List[Dict[str, Any]]) -> str:
        ...
        for trade in trades:
            ...
            gain_loss = trade.get("realized_pnl", trade.get("pnl", proceeds - cost_basis))
    ...
    def generate_tax_summary(self, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for trade in trades:
            date_acquired = self._parse_date(trade.get("entry_time", trade.get("timestamp")))
        ...
        return {
            ...
            "short_term": {
                ...
                "gains": sum(t.get("realized_pnl", t.get("pnl", 0)) for t in short_term_trades if t.get("realized_pnl", t.get("pnl", 0)) > 0),
                "losses": sum(t.get("realized_pnl", t.get("pnl", 0)) for t in short_term_trades if t.get("realized_pnl", t.get("pnl", 0)) < 0),
                "estimated_tax": estimated_st_tax,
            },
            "long_term": {
                "trades": len(long_term_trades),
                "total_gain_loss": long_term_gain,
                "gains": sum(t.get("realized_pnl", t.get("pnl", 0)) for t in long_term_trades if t.get("realized_pnl", t.get("pnl", 0)) > 0),
                "losses": sum(t.get("realized_pnl", t.get("pnl", 0)) for t in long_term_trades if t.get("realized_pnl", t.get("pnl", 0)) < 0),
