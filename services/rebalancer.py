...
class PortfolioRebalancer:
    ...
    def __init__(
        self, min_allocation_pct: float = 5.0, max_allocation_pct: float = 40.0
    ):
        """
    ...
    def rebalance(self, strategies: List[str] = None, lookback_days: int = 30) -> Dict:
        """
        ...
        if not strategy_performance:
            return {"success": False, "error": "No strategy performance data available"}

        # Filter to specified strategies if provided
        if strategies:
            strategy_performance = {
                k: v for k, v in strategy_performance.items() if k in strategies
            }
            ...
            rebalancing_plan.append(
                {
                    ...
                    "current_allocation_pct": (current_allocation / portfolio_value)
                    * 100,
                    "target_allocation": target_value,
                    "target_allocation_pct": allocation_pct,
                    "change": change,
                    "change_pct": (
                        ((target_value - current_allocation) / current_allocation * 100)
    ...
    def _calculate_strategy_scores(self, strategy_performance: Dict) -> Dict:
        ...
        for strategy, data in strategy_performance.items():
            ...
            if closed_positions == 0:
                ...
                sharpe_ratio = (
                    avg_return * 10 if data["win_rate"] > 50 else avg_return * 5
                )
            ...
            combined_score = sharpe_score * 0.4 + win_rate_score * 0.3 + pnl_score * 0.3
    ...
    def _calculate_optimal_allocations(self, performances: Dict) -> Dict[str, float]:
        ...
        if total_allocation > 0:
            allocations = {
                k: (v / total_allocation) * 100.0 for k, v in allocations.items()
            }
    ...
    def update_constraints(
        self, min_allocation_pct: float = None, max_allocation_pct: float = None
    ):
        """
    ...
    def schedule_rebalancing(
        self, frequency: str = "weekly", day_of_week: int = 0, hour: int = 0
    ) -> Dict:
        """
        ...
        self.logger.info(f"Rebalancing scheduled: {frequency} at {hour:02d}:00")

        return schedule_info

    def _calculate_next_rebalance_time(
        self, frequency: str, day_of_week: int, hour: int
    ) -> str:
