...
class RiskManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Position sizing
        self.default_position_pct = risk_config.get("default_position_pct", 0.1)  # 10% of capital
        self.max_position_pct = risk_config.get("max_position_pct", 0.2)  # 20% of capital
        
        # Stop-loss and take-profit
        self.default_stop_loss_pct = risk_config.get("default_stop_loss_pct", 0.05)  # 5%
        self.default_take_profit_pct = risk_config.get("default_take_profit_pct", 0.10)  # 10%
    ...
    def check_can_trade(self, position_size: float) -> Tuple[bool, Optional[str]]:
        ...
        if position_size > self.max_position_size:
            return False, f"Position size ${position_size:.2f} exceeds limit ${self.max_position_size:.2f}"
        
        # Check total exposure
        total_exposure = sum(
            pos["size"] for pos in self.open_positions.values()
        )
        
        if total_exposure + position_size > self.max_total_exposure:
            return False, f"Total exposure would exceed limit (${total_exposure + position_size:.2f} > ${self.max_total_exposure:.2f})"
    ...
    def record_trade(
        ...
        logger.info(f"Recorded trade: {market_id} {direction} ${size:.2f} @ {entry_price:.4f}")
    ...
    def record_pnl(self, pnl: float):
        ...
        if pnl < 0:
            ...
            logger.info(f"Recorded loss: ${abs(pnl):.2f}, daily loss: ${self.daily_loss:.2f}")
    ...
    def check_stop_loss_take_profit(
        ...
        if direction in ["long", "buy"]:
            if current_price <= stop_loss:
                logger.warning(f"Stop-loss triggered for {market_id}: {current_price:.4f} <= {stop_loss:.4f}")
                return "stop_loss"
            
            if current_price >= take_profit:
                logger.info(f"Take-profit triggered for {market_id}: {current_price:.4f} >= {take_profit:.4f}")
                return "take_profit"
    ...
    def get_risk_summary(self) -> Dict[str, Any]:
        ...
        total_unrealized_pnl = sum(pos.get("unrealized_pnl", 0) for pos in self.open_positions.values())
        ...
        return {
            ...
            "can_trade": self.daily_loss < self.max_daily_loss and len(self.open_positions) < self.max_positions,
        }
