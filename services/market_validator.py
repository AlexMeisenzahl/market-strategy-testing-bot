...
class MarketValidator:
    ...
    def validate_market_against_reality(self, market: Dict, current_prices: Dict[str, Dict]) -> Optional[Dict]:
        """
        ...
        if reality_met:
            ...
            distance_pct = abs(float(current_price - threshold)) / float(threshold) * 100
    ...
    def _extract_crypto_info(self, market_name: str) -> Optional[Tuple[str, Decimal, str]]:
        """
        ...
        direction = None
        if any(word in market_lower for word in ['above', 'over', 'higher', 'exceed', '>']):
            direction = 'above'
        elif any(word in market_lower for word in ['below', 'under', 'lower', 'less', '<']):
            direction = 'below'
    ...
    def _check_condition(self, current_price: Decimal, threshold: Decimal, direction: str) -> bool:
        """
