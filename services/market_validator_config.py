...
def get_discrepancy_level(
    current_price: float, threshold: float, market_yes_price: float, direction: str
) -> Dict[str, Any]:
    """
    ...
    if is_near_threshold:
        ...
        return {
            ...
            "discrepancy": abs(
                NEAR_THRESHOLD_CONFIG["expected_market_price"] - market_yes_price
            ),
    ...
    if level != DiscrepancyLevel.NONE and level != DiscrepancyLevel.LOW:
        if reality_is_yes:
            profit_potential = (
                (1 - market_yes_price) / market_yes_price if market_yes_price > 0 else 0
            )
        else:
            profit_potential = (
                (market_yes_price) / (1 - market_yes_price)
                if market_yes_price < 1
                else 0
            )
    else:
        profit_potential = 0.0
...
def _get_reason(
    reality_is_yes: bool, current_price: float, threshold: float, direction: str
) -> str:
    """
