...
class TimeAnalytics:
    ...
    def get_hour_of_day_analysis(
        ...
        for hour in hours:
            ...
            if hour_trades:
                ...
                win_rate = float(
                    (Decimal(winning_count) / Decimal(trade_count) * Decimal('100')).quantize(
    ...
    def get_best_trading_times(
        ...
        # Find top 5 hours
        hour_pnl_pairs = list(zip(hour_analysis['hours'], hour_analysis['pnl_per_hour']))
        top_hours = sorted(hour_pnl_pairs, key=lambda x: x[1], reverse=True)[:5]
        
        # Find top 3 days
        day_pnl_pairs = list(zip(day_analysis['day_names'], day_analysis['pnl_per_day']))
        top_days = sorted(day_pnl_pairs, key=lambda x: x[1], reverse=True)[:3]
        
        return {
            'top_hours': [
                {'hour': f"{h}:00", 'pnl': round(p, 2)} 
