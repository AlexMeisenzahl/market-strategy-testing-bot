...
class PerformanceCalculator:
    ...
    def calculate_profit_factor(self, trades: List[Dict[str, Any]]) -> float:
        ...
        gross_profit = sum(self._get_trade_pnl(t) for t in trades if self._get_trade_pnl(t) > 0)
        gross_loss = abs(sum(self._get_trade_pnl(t) for t in trades if self._get_trade_pnl(t) < 0))
    ...
    def calculate_cagr(self, returns: List[float], trades: List[Dict[str, Any]]) -> float:
        """
    ...
    def calculate_avg_win(self, trades: List[Dict[str, Any]]) -> float:
        ...
        Returns:
            Average win amount
        """
        winning_trades = [self._get_trade_pnl(t) for t in trades if self._get_trade_pnl(t) > 0]
    ...
    def calculate_avg_loss(self, trades: List[Dict[str, Any]]) -> float:
        ...
        """
        losing_trades = [abs(self._get_trade_pnl(t)) for t in trades if self._get_trade_pnl(t) < 0]
