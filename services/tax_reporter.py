...
class TaxReporter:
    ...
    def _calculate_fifo(
        ...
        for sell in sells:
            ...
            transactions.append({
                ...
                'acquisition_date': acquisition_date.strftime('%m/%d/%Y') if acquisition_date else 'VARIOUS',
                'sale_date': sell_date.strftime('%m/%d/%Y') if sell_date else 'UNKNOWN',
    ...
    def _generate_csv_report(
        ...
        writer.writerow(['Total Gain/Loss:', f"${(short_term_total + long_term_total):.2f}"])
        writer.writerow([])
        writer.writerow(['Note: This is a simplified report. Consult a tax professional for accurate filing.'])
