...
class RealityArbitrageDetector:
    ...
    def __init__(self, logger=None, config: Dict = None):
        ...
        # Configuration
        reality_config = self.config.get('strategies', {}).get('polymarket_arbitrage', {}).get('arbitrage_types', {}).get('reality_based', {})
        self.enabled = reality_config.get('enabled', True)
        self.min_profit_pct = Decimal(str(reality_config.get('min_profit_pct', 5.0)))
        self.min_confidence = reality_config.get('min_confidence', 'high')  # 'medium', 'high', 'very_high'
    ...
    def check_all_markets(self, markets: List[Dict]) -> List[Dict]:
        ...
        crypto_markets = [m for m in markets if self.validator.is_crypto_market(m.get('market_name', ''))]
        ...
        if self.logger:
            self.logger.log_info(f"Checking {len(crypto_markets)} crypto markets for reality arbitrage")
        ...
        for market in crypto_markets:
            validation = self.validator.validate_market_against_reality(market, current_prices)
            ...
            if not validation['valid'] and validation['opportunity']:
                ...
                opportunity = {
                    ...
                    'market_no_price': market.get('no_price', 1 - validation['market_yes_price']),
        ...
        if self.logger and opportunities:
            self.logger.log_info(f"Found {len(opportunities)} reality arbitrage opportunities")
