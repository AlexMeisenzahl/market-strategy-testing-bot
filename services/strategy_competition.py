...
class StrategyCompetition:
    ...
    def _execute_virtual_trade(self, strategy_id: int, opportunity: Dict):
        ...
        try:
            ...
            # Update total value
            positions_value = sum(
                p["size"] * p["entry_price"] for p in portfolio["positions"]
            )
    ...
    def _calculate_performance_metrics(self, strategy_id: int) -> Dict:
        ...
        win_rate = (
            (portfolio["wins"] / total_closed_trades * 100)
    ...
    def get_competition_summary(self) -> Dict:
        ...
        avg_return = (
            np.mean([s["return_pct"] for s in leaderboard]) if leaderboard else 0
        )
