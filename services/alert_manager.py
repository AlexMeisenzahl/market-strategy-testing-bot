...
class AlertManager:
    ...
    def create_alert(self, alert_type: str, condition: Dict,
        ...
        self.logger.info(
            f"Alert created: #{alert_id} ({alert_type}) - {condition}"
        )
    ...
    def check_alerts(self, current_data: Dict = None) -> List[Dict]:
        ...
        for alert in enabled_alerts:
            ...
            if should_trigger:
                ...
                self.logger.info(
                    f"Alert triggered: #{alert['id']} ({alert_type})"
                )
    ...
    def _send_alert_notification(self, alert: Dict, condition: Dict):
        ...
        if alert_type == 'price':
            ...
            message = (
                f"Strategy Alert: {condition['strategy']} "
    ...
    def get_alert_statistics(self) -> Dict:
        ...
        most_triggered = sorted(
            alerts,
            key=lambda a: a.get('trigger_count', 0),
            reverse=True
        )[:5]
