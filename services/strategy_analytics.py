...
class StrategyAnalytics:
    ...
    def _calculate_metrics(self, strategy_name: str, trades: List[Dict]) -> Dict[str, Any]:
        ...
        # Win rate
        win_rate = (Decimal(winning_count) / Decimal(total_trades) * Decimal('100')).quantize(
            Decimal('0.01'), rounding=ROUND_HALF_UP
        ) if total_trades > 0 else Decimal('0')
        
        # P&L calculations using Decimal
        total_pnl = sum(Decimal(str(t['pnl_usd'])) for t in trades)
        avg_profit = (total_pnl / Decimal(total_trades)).quantize(
            Decimal('0.01'), rounding=ROUND_HALF_UP
        ) if total_trades > 0 else Decimal('0')
        
        # Average win and loss
        avg_win = (
            sum(Decimal(str(t['pnl_usd'])) for t in winning_trades) / Decimal(winning_count)
        ).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP) if winning_count > 0 else Decimal('0')
        
        avg_loss = (
            sum(Decimal(str(t['pnl_usd'])) for t in losing_trades) / Decimal(losing_count)
        ).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP) if losing_count > 0 else Decimal('0')
        ...
        largest_win = max(Decimal(str(t['pnl_usd'])) for t in trades) if trades else Decimal('0')
        largest_loss = min(Decimal(str(t['pnl_usd'])) for t in trades) if trades else Decimal('0')
        
        # Average trade duration in hours
        total_duration_minutes = sum(t['duration_minutes'] for t in trades)
        avg_duration_hours = (
            Decimal(str(total_duration_minutes)) / Decimal('60') / Decimal(total_trades)
        ).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP) if total_trades > 0 else Decimal('0')
    ...
    def _calculate_max_consecutive(self, trades: List[Dict], winning: bool = True) -> int:
        """
