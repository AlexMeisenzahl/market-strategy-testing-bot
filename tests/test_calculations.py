...
class TestCalculations:
    ...
    def test_average_pnl(self):
        ...
        trades = [{"pnl_usd": 10.00}, {"pnl_usd": 20.00}, {"pnl_usd": 30.00}]
    ...
    def test_daily_pnl_chart_data(self):
        ...
        assert (
            len(result["labels"]) == 3
        ), f"Expected 3 days, got {len(result['labels'])}"
        assert (
            len(result["data"]) == 3
        ), f"Expected 3 data points, got {len(result['data'])}"
    ...
    def test_cumulative_pnl_chart_data(self):
        ...
        for i, (actual, expected) in enumerate(
            zip(result["data"], expected_cumulative)
        ):
            assert (
                abs(actual - expected) < 0.01
        ...
        assert (
            abs(final_point - total_pnl) < 0.01
        ), f"Final cumulative ({final_point}) doesn't match total P&L ({total_pnl})"

        print(
            f"✓ test_cumulative_pnl_chart_data: {expected_cumulative}, final={total_pnl}"
        )
    ...
    def test_decimal_precision(self):
        ...
        trades = [{"pnl_usd": 0.1}, {"pnl_usd": 0.2}]
    ...
    def test_dates_unique_in_daily_chart(self):
        ...
        assert len(result["labels"]) == len(
            set(result["labels"])
        ), "Dates are not unique!"

        # Feb 1 should sum to 18, Feb 2 should be -2
        assert (
            abs(result["data"][0] - 18.0) < 0.01
        ), f"Expected 18.0 for Feb 1, got {result['data'][0]}"
        assert (
            abs(result["data"][1] - (-2.0)) < 0.01
        ), f"Expected -2.0 for Feb 2, got {result['data'][1]}"

        print(
            f"✓ test_dates_unique_in_daily_chart: 2 unique dates, aggregated correctly"
        )
