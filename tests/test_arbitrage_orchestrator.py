...
class TestArbitrageOrchestrator:
    ...
    def test_orchestrator_initializes_tracker(self):
        ...
        assert (
            self.orchestrator.tracker.opportunities_found == 0
        ), "Should start with 0 opportunities"
    ...
    def test_detect_opportunity_calls_tracker(self):
        ...
        market_data = {"id": "test_market_1", "question": "Test Market Question"}
        ...
        if opportunity:
            assert (
                self.orchestrator.tracker.opportunities_found == initial_count + 1
            ...
            assert (
                self.orchestrator.tracker.opportunities_found == initial_count
            ), "Tracker should not increment when no opportunity"
            print(
                "✓ test_detect_opportunity_calls_tracker: Tracker not incremented (no opportunity)"
            )
    ...
    def test_detect_opportunity_returns_none_for_no_arbitrage(self):
        ...
        price_data = {"yes": 0.55, "no": 0.50}

        opportunity = self.orchestrator.detect_opportunity(market_data, price_data)

        assert opportunity is None, "Should return None when no arbitrage opportunity"
        print(
            "✓ test_detect_opportunity_returns_none_for_no_arbitrage: Correctly returns None"
        )
    ...
    def test_execute_opportunity_calls_tracker(self):
        ...
        if opportunity:
            ...
            # Verify tracker was updated
            assert (
                self.orchestrator.tracker.opportunities_executed
            ...
            print(
                "✓ test_execute_opportunity_calls_tracker: Tracker updated after execution"
            )
        else:
            print(
                "✓ test_execute_opportunity_calls_tracker: Skipped (no opportunity detected)"
            )
    ...
    def test_full_workflow_tracks_correctly(self):
        ...
        # Setup test data
        market_data_1 = {"id": "market_1", "question": "Test Market 1"}
        price_data_1 = {"yes": 0.40, "no": 0.45}

        market_data_2 = {"id": "market_2", "question": "Test Market 2"}
        price_data_2 = {"yes": 0.42, "no": 0.46}
        ...
        # Verify tracking
        assert (
            final_metrics["opportunities_found"] == opportunities_found
        ), f"Should have found {opportunities_found} opportunities"
        assert (
            final_metrics["opportunities_executed"] == executions_count
        ...
        print(
            f"✓ test_full_workflow_tracks_correctly: Found {opportunities_found}, Executed {executions_count}"
        )

    def test_print_performance_dashboard(self):
        """Test dashboard printing doesn't raise errors"""
        # Add some data
        market_data = {"id": "test_market", "question": "Test Market"}
        price_data = {"yes": 0.40, "no": 0.45}
    ...
    def test_reset_performance_tracking(self):
        ...
        metrics_before = self.orchestrator.get_performance_metrics()
        has_data = (
            metrics_before["opportunities_found"] > 0
            or metrics_before["opportunities_executed"] > 0
        )
        ...
        if has_data:
            ...
            print(
                "✓ test_reset_performance_tracking: Reset verified (no data to reset)"
            )
