...
class NotificationChannelFactory(factory.Factory):
    ...
    user_id = 1
    channel_type = factory.Iterator(
        ["discord", "slack", "email", "telegram", "webhook"]
    )
    enabled = True
    webhook_url = factory.LazyAttribute(
        lambda obj: (
            f"https://hooks.example.com/{obj.channel_type}/{factory.Faker('uuid4').generate({})}"
            if obj.channel_type in ["discord", "slack", "webhook"]
            else None
        )
    )
    api_key = None
    email_address = factory.LazyAttribute(
        lambda obj: (
            factory.Faker("email").generate({}) if obj.channel_type == "email" else None
        )
    )
    phone_number = None
    config_json = None
...
class OpportunityFactory(factory.Factory):
    ...
    liquidity_usd = factory.Faker(
        "pydecimal", left_digits=5, right_digits=0, positive=True
    )
...
class CryptoPriceFactory(factory.Factory):
    ...
    volume_24h = factory.Faker(
        "pydecimal", left_digits=10, right_digits=0, positive=True
    )
    market_cap = factory.Faker(
        "pydecimal", left_digits=12, right_digits=0, positive=True
    )
...
class AuditLogFactory(factory.Factory):
    ...
    entity_type = factory.Iterator(
        ["user_settings", "strategy", "notification_channel", "system_config"]
    )
