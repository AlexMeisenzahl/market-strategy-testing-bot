...
class MockDataParser:
    ...
    def get_trades(self, strategy=None, symbol=None, start_date=None, end_date=None, per_page=100):
        """Return mock trades"""
...
class TestStrategyAnalytics:
    ...
    def test_strategy_performance_metrics(self):
        ...
        required_keys = [
            ...
            'win_rate', 'total_pnl', 'avg_profit', 'avg_win', 'avg_loss',
            'profit_factor', 'largest_win', 'largest_loss', 'avg_duration_hours',
    ...
    def test_win_rate_calculation(self):
        ...
        print(f"✓ test_win_rate_calculation: {metrics['win_rate']} ≈ {expected_win_rate}")
    ...
    def test_profit_factor_calculation(self):
        ...
        print(f"✓ test_profit_factor_calculation: {metrics['profit_factor']} ≈ {expected_pf}")
    ...
    def test_expectancy_calculation(self):
        ...
        print(f"✓ test_expectancy_calculation: {metrics['expectancy']} ≈ {expected_expectancy}")
    
    def test_consecutive_wins(self):
        """Test max consecutive wins calculation"""
        # Create trades with consecutive wins
        consecutive_trades = [
            {'id': 1, 'strategy': 'Test', 'symbol': 'A', 'entry_time': '2024-01-01T10:00:00',
             'duration_minutes': 60, 'pnl_usd': 10.0, 'outcome': 'win'},
            {'id': 2, 'strategy': 'Test', 'symbol': 'A', 'entry_time': '2024-01-01T11:00:00',
             'duration_minutes': 60, 'pnl_usd': 20.0, 'outcome': 'win'},
            {'id': 3, 'strategy': 'Test', 'symbol': 'A', 'entry_time': '2024-01-01T12:00:00',
             'duration_minutes': 60, 'pnl_usd': 30.0, 'outcome': 'win'},
            {'id': 4, 'strategy': 'Test', 'symbol': 'A', 'entry_time': '2024-01-01T13:00:00',
             'duration_minutes': 60, 'pnl_usd': -10.0, 'outcome': 'loss'},
            {'id': 5, 'strategy': 'Test', 'symbol': 'A', 'entry_time': '2024-01-01T14:00:00',
             'duration_minutes': 60, 'pnl_usd': 15.0, 'outcome': 'win'},
        ]
    ...
    def test_max_drawdown(self):
        ...
        dd_trades = [
            {'id': 1, 'strategy': 'Test', 'symbol': 'A', 'entry_time': '2024-01-01T10:00:00',
             'duration_minutes': 60, 'pnl_usd': 100.0, 'outcome': 'win'},
            {'id': 2, 'strategy': 'Test', 'symbol': 'A', 'entry_time': '2024-01-01T11:00:00',
             'duration_minutes': 60, 'pnl_usd': 50.0, 'outcome': 'win'},
            {'id': 3, 'strategy': 'Test', 'symbol': 'A', 'entry_time': '2024-01-01T12:00:00',
             'duration_minutes': 60, 'pnl_usd': -100.0, 'outcome': 'loss'},
            {'id': 4, 'strategy': 'Test', 'symbol': 'A', 'entry_time': '2024-01-01T13:00:00',
             'duration_minutes': 60, 'pnl_usd': -50.0, 'outcome': 'loss'},
        ]
        ...
        expected_dd = 150.0
        assert abs(metrics['max_drawdown'] - expected_dd) < 0.1, \
    ...
    def test_all_strategies_performance(self):
        ...
        assert all_strategies[0]['total_pnl'] >= all_strategies[1]['total_pnl'], \
            "Strategies not sorted by P&L descending"
        
        print(f"✓ test_all_strategies_performance: {len(all_strategies)} strategies, sorted")
...
class TestMarketAnalytics:
    ...
    def setup_method(self):
        """Setup test environment"""
        self.sample_trades = [
            {'id': 1, 'strategy': 'A', 'symbol': 'AAPL', 'entry_time': '2024-01-01T10:00:00',
             'duration_minutes': 60, 'pnl_usd': 100.0, 'outcome': 'win'},
            {'id': 2, 'strategy': 'A', 'symbol': 'AAPL', 'entry_time': '2024-01-01T11:00:00',
             'duration_minutes': 60, 'pnl_usd': 50.0, 'outcome': 'win'},
            {'id': 3, 'strategy': 'A', 'symbol': 'AAPL', 'entry_time': '2024-01-01T12:00:00',
             'duration_minutes': 60, 'pnl_usd': -25.0, 'outcome': 'loss'},
            {'id': 4, 'strategy': 'B', 'symbol': 'TSLA', 'entry_time': '2024-01-01T13:00:00',
             'duration_minutes': 60, 'pnl_usd': 75.0, 'outcome': 'win'},
            {'id': 5, 'strategy': 'B', 'symbol': 'TSLA', 'entry_time': '2024-01-01T14:00:00',
             'duration_minutes': 60, 'pnl_usd': -30.0, 'outcome': 'loss'},
            {'id': 6, 'strategy': 'B', 'symbol': 'TSLA', 'entry_time': '2024-01-01T15:00:00',
             'duration_minutes': 60, 'pnl_usd': -20.0, 'outcome': 'loss'},
        ]
    ...
    def test_market_win_rate(self):
        ...
        expected_wr = 66.67
        assert abs(aapl['win_rate'] - expected_wr) < 0.1, \
    ...
    def test_market_frequency(self):
        ...
        aapl = next(m for m in markets if m['market'] == 'AAPL')
        expected_freq = 50.0
        assert abs(aapl['frequency'] - expected_freq) < 0.1, \
    ...
    def test_min_trades_filter(self):
        ...
        assert len(markets) == 0, f"Expected 0 markets with min_trades=5, got {len(markets)}"
...
class TestTimeAnalytics:
    ...
    def test_hour_grouping(self):
        ...
        assert hour_10_trades == 3, f"Expected 3 trades at hour 10, got {hour_10_trades}"
    ...
    def test_best_trading_times(self):
        ...
        print(f"✓ test_best_trading_times: {len(best_times['top_hours'])} hours, {len(best_times['top_days'])} days")
...
class TestRiskMetrics:
    ...
    def setup_method(self):
        ...
        self.sample_trades = [
            {'id': 1, 'strategy': 'A', 'symbol': 'AAPL', 'entry_time': '2024-01-01T10:00:00',
             'duration_minutes': 60, 'pnl_usd': 100.0, 'pnl_pct': 2.0, 'outcome': 'win'},
            {'id': 2, 'strategy': 'A', 'symbol': 'AAPL', 'entry_time': '2024-01-02T10:00:00',
             'duration_minutes': 60, 'pnl_usd': -50.0, 'pnl_pct': -1.0, 'outcome': 'loss'},
            {'id': 3, 'strategy': 'A', 'symbol': 'AAPL', 'entry_time': '2024-01-03T10:00:00',
             'duration_minutes': 60, 'pnl_usd': 75.0, 'pnl_pct': 1.5, 'outcome': 'win'},
            {'id': 4, 'strategy': 'A', 'symbol': 'AAPL', 'entry_time': '2024-01-04T10:00:00',
             'duration_minutes': 60, 'pnl_usd': 50.0, 'pnl_pct': 1.0, 'outcome': 'win'},
            {'id': 5, 'strategy': 'A', 'symbol': 'AAPL', 'entry_time': '2024-01-05T10:00:00',
             'duration_minutes': 60, 'pnl_usd': -25.0, 'pnl_pct': -0.5, 'outcome': 'loss'},
        ]
    ...
    def test_all_risk_metrics_present(self):
        ...
        required_keys = [
            'sharpe_ratio', 'sortino_ratio', 'var_95', 'var_99',
            'max_drawdown', 'max_drawdown_pct', 'calmar_ratio',
            'std_deviation', 'downside_deviation', 'annualized_return',
    ...
    def test_var_calculation(self):
        ...
        print(f"✓ test_var_calculation: VaR 95%=${metrics['var_95']}, VaR 99%=${metrics['var_99']}")
