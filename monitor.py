...
class PolymarketMonitor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.rate_limiter = RateLimiter(
            max_requests=config.get("rate_limit_max", 100), time_window=60
        )
    ...
    def check_connection_health(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"{self.BASE_URL}{self.MARKETS_ENDPOINT}", timeout=self.timeout
            )
        ...
        if self.live_api_enabled and self.api:
            try:
                ...
                self.logger.log_warning(
                    f"Live API failed, falling back to Subgraph: {e}"
                )
    ...
    def _get_live_market_prices(self, market_id: str) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            self.logger.log_error(
                f"Error fetching live prices for {market_id}: {str(e)}"
            )
            return None

    def _get_simulated_market_prices(
        self, market_id: str
