...
def test_telegram_bot_class():
    ...
    assert (
        "def send_message(self, text: str)" in content
    ), "send_message method not found"
...
def test_telegram_integration_in_run_bot():
    ...
    assert (
        "self.telegram_bot = SimpleTelegramBot" in content
    ...
    assert (
        'os.getenv("TELEGRAM_BOT_TOKEN"' in content
        or "os.getenv('TELEGRAM_BOT_TOKEN'" in content
    ), "Environment variable not checked"
...
def test_alert_integration():
    ...
    assert (
        "from services.alert_manager import alert_manager" in content
    ), "Alert manager not imported"
...
def test_settings_integration():
    ...
    assert (
        "from services.settings_manager import get_settings_manager" in content
    ), "Settings manager not imported"
    assert (
        "self.settings_manager = get_settings_manager" in content
    ), "Settings manager not initialized"
    assert (
        "self.cycle_interval = self.settings_manager.get_setting" in content
    ), "Cycle interval not configured"
...
def test_template_javascript():
    ...
    assert (
        "function testAlert()" in alerts_content
    ), "testAlert() function not found in alerts.html"
    assert (
        "/api/alerts/test" in alerts_content
    ), "testAlert() doesn't call correct endpoint"
    ...
    assert (
        "function testKey(service)" in api_keys_content
    ), "testKey() function not found in api_keys.html"
    assert (
        "/api/keys/test" in api_keys_content
    ), "testKey() doesn't call correct endpoint"
