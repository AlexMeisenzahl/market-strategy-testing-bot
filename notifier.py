...
class Notifier:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.desktop_enabled = desktop_config.get(
            "enabled", config.get("desktop_notifications", False)
        )
        self.desktop_event_types = desktop_config.get(
            "event_types",
            {
                "trade": True,
                "opportunity": True,
        ...
        self.email_event_types = email_notif_config.get(
            "event_types",
            {
                "trade": True,
        ...
        self.telegram_event_types = telegram_notif_config.get(
            "event_types",
            {
                "trade": True,
                "opportunity": True,
    ...
    def alert_opportunity_found(self, market: str, profit_pct: float) -> None:
        ...
        if trigger_config.get("enabled", True):
            ...
            if profit_pct >= min_profit:
                ...
                self.notify(
                    title, message, priority="CRITICAL", event_type="opportunity"
                )
                return
    ...
    def test_notifications(self) -> Dict[str, bool]:
        ...
        if self.desktop_enabled:
            results["desktop"] = self.send_desktop_notification(
                "Test Notification", "This is a test of the desktop notification system"
            )
        ...
        if self.telegram_enabled:
            results["telegram"] = self.send_push(
                "Test Push", "This is a test of the Telegram notification system"
            )
