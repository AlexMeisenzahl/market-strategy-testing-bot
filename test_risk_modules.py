...
def test_strategy_analyzer(config):
    ...
    if rankings:
        print(f"  #1 strategy: {rankings[0]['strategy']} (score: {rankings[0]['overall_score']:.1f})")
    
    # Generate allocation
    allocation = analyzer.generate_allocation_recommendation(comparison, total_capital=1000)
    print(f"✓ Allocation recommendation generated")
    for strategy_name, alloc in allocation['allocations'].items():
        print(f"  {strategy_name}: {alloc['percentage']:.1f}% (${alloc['capital']:.2f})")
