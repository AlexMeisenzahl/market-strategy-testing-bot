    ...
    def _check_rate_limit(self) -> bool:
        ...
        if self._notification_count >= max_per_minute:
            self.logger.log_warning(
                "Telegram rate limit reached, skipping notification"
            )
            return False
...
Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
    ...
    def send_profit_loss_update(
        self, daily_pnl: float, daily_pnl_pct: float, total_pnl: float, win_rate: float
    ) -> bool:
        """
...
Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
    ...
    def send_strategy_triggered(
        self, strategy_name: str, market_name: str, confidence: float, action: str
    ) -> bool:
        """
...
Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
    ...
    def send_status_change(self, status: str, message: str) -> bool:
        """
...
def create_notifications_service(
    notifier: TelegramNotifier,
) -> Optional[TelegramNotifications]:
    """
