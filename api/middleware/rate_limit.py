...
class RateLimiter:
    ...
    def _cleanup_old_requests(self):
        ...
        for ip in list(self.requests.keys()):
            self.requests[ip] = [
                (ts, count) for ts, count in self.requests[ip] if ts > cutoff
            ]
    ...
    async def check_rate_limit(self, ip: str) -> Tuple[bool, str]:
        ...
        with self._lock:  # Thread-safe access to shared state
            ...
            # Get recent requests
            recent_requests = [
                (ts, count) for ts, count in self.requests[ip] if ts > minute_ago
            ]
            ...
            burst_requests = sum(
                count for ts, count in recent_requests if ts > ten_seconds_ago
            )
            ...
            if burst_requests >= self.burst:
                return (
                    False,
                    f"Burst limit exceeded ({self.burst} requests per 10 seconds)",
                )
...
def setup_rate_limiting(app: FastAPI, requests_per_minute: int = 60, burst: int = 10):
    ...
    logger.log_info(
        f"Rate limiting enabled: {requests_per_minute} req/min, burst: {burst}"
    )
