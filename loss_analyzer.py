...
class LossAnalyzer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.execution_lag_threshold_ms = analysis_config.get('execution_lag_threshold_ms', 500)
        self.partial_fill_threshold = analysis_config.get('partial_fill_threshold', 0.8)  # 80%
        ...
        for category, trades in categories.items():
            if trades:
                analysis['categories'][category] = {
                    ...
                    'percentage': (len(trades) / len(losing_trades)) * 100 if losing_trades else 0,
                    'trades': trades
                }
    ...
    def _categorize_loss(self, trade: Dict[str, Any], full_history: List[Dict[str, Any]]) -> str:
        """
    ...
    def _is_strategy_failure(self, trade: Dict[str, Any], full_history: List[Dict[str, Any]]) -> bool:
        """
    ...
    def _is_market_condition_issue(self, trade: Dict[str, Any], full_history: List[Dict[str, Any]]) -> bool:
        """
    ...
    def _is_competition_issue(self, trade: Dict[str, Any], full_history: List[Dict[str, Any]]) -> bool:
        """
        ...
        if len(recent_trades) >= 20:
            ...
            avg_margin_first = sum(t.get('profit_margin', 0) for t in first_half) / len(first_half)
            avg_margin_second = sum(t.get('profit_margin', 0) for t in second_half) / len(second_half)
    ...
    def generate_fix_suggestions(self, analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
    ...
    def _log_analysis(self, analysis: Dict[str, Any]) -> None:
        ...
        self.logger.log_critical(
            f"Total Loss: ${abs(analysis['total_loss']):.2f}"
        )
        
        if analysis['primary_issue']:
            self.logger.log_critical(
                f"PRIMARY ISSUE: {analysis['primary_issue']}"
            )
